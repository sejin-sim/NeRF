...
if __name__ == '__main__':
    hparams = get_opts()         # 입력한 값들을 저장하고
    system = NeRFSystem(hparams) # NeRF 모델을 지정한 값들 기준으로 생성

    # 체크포인트 콜백을 생성
    checkpoint_callback = ModelCheckpoint(filepath=os.path.join(f'ckpts/{hparams.exp_name}',
                                                                '{epoch:d}'),
                                          monitor='val/loss', # val/loss를 기준으로
                                          mode='min',         # 작은 경우
                                          save_top_k=5,)      # 최종 top 5의 ckpt만 저장
    ...
    trainer = Trainer(max_epochs=hparams.num_epochs,
                      ...
                      distributed_backend='ddp' if hparams.num_gpus>1 else None, # 분산 훈련 DistributedDataParalle
                      num_sanity_val_steps=1,
                      benchmark=True,
                      profiler=hparams.num_gpus==1)

    trainer.fit(system) # model을 넣고 fit
